...
pub struct Morphology {
    ...
    pub design_mass: f64,
    /// A recipe the engine generated by assessing what this thing is made of —
    /// `docs/PLAY.md` D15's composite, which is one node and a parts list
    /// rather than a pile of promoted nodes.
    ///
    /// **Present, it decides the shape; absent, `program` does.** The two are
    /// not alternatives and that is the point. A wooden box is a tree, cut into
    /// pieces, carved, and attached together: `program` records what the
    /// material *is* and therefore what it weighs, what it is made of and what
    /// burning it releases, while the assembly records what was built out of
    /// it. Nothing had to be added to the species table to describe a box,
    /// which is what D11 is for.
...
impl Morphology {
    ...
    pub fn assembled(
        ...
        m.progress = 1.0;
        m.assembly = Some(parts);
        m
    }

    /// Is this thing's shape a generated recipe rather than its program's?
    pub fn is_assembled(&self) -> bool {
        self.assembly.as_ref().is_some_and(|a| !a.is_empty())
    }
    ...
    pub fn density(&self) -> f64 {
        if let Some(a) = self.assembly.as_ref() {
    ...
    pub fn extent(&self) -> f64 {
        // An assembled thing is the size its parts are. A grown one derives its
        // size from the mass it accumulated, which is the arrow pointing the
        // other way and the reason this is a branch rather than a formula.
        if let Some(a) = self.assembly.as_ref() {
            if !a.is_empty() {
                return a.extent().max(1e-9);
            }
        }
        match self.program {
    ...
    pub fn render(&self, budget: usize) -> Skeleton {
        // The parts *are* the geometry, so there is no budget to spend and no
        // level of detail to choose: a six-panel box is six members whoever is
        // looking. A budget smaller than the parts list is a caller asking for
        // something the recipe cannot honestly give, and the recipe wins.
        if let Some(a) = self.assembly.as_ref() {
            if !a.is_empty() {
                return a.render();
            }
