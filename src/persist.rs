...
//! sampling is not deterministic or the epoch bookkeeping is wrong, and
...
pub(crate) fn put_spec(w: &mut Writer, s: &SampleSpec) {
...
pub(crate) fn get_spec(r: &mut Reader) -> Result<SampleSpec> {
    Ok(SampleSpec {
...
pub(crate) fn put_node_payload(w: &mut Writer, n: &Node) {
    ...
    //
    // It travels because it is an *input to regeneration*. A structure is
    // proportioned against its own weight, so its members' radii depend on the
    // field it was built in, and a client that worked the field out for itself
