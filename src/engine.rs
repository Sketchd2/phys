...
impl World {
    ...
    pub fn node_dt(&self, idx: NodeIdx) -> f64 {
        ...
        if let Some(c) = n.ground.as_ref() {
            natural = natural.min(solvers::ground::stable_step(&n.bodies, &c.springs, &c.foundation));
    ...
    ///
    /// **The ocean is a node** — the owner's decision for Phase 5: a child of
    /// its planet, a shell whose matter is the sea's water and whose
    /// description is the ocean's cells (`Node::ocean`). Its water comes out
    /// of the planet's own bodies, each by its share of their mass and with
    /// what that share carries (`Tree::withdraw`), so making it moves nothing
    /// in the world's books; and it is never drawn as bodies of its own
    /// (`Tree::refine`).
    ///
    /// **Only over ground.** An ocean is water standing on rock, so a planet
    /// is asked for one once what its ground is has been written down — its
    /// surface (`World::assess_surface`) or the layout its freezing laid down
    /// (`Recipe::Granular`) — and not before: a surface written afterwards
    /// would redraw the planet's bodies under a sea already taken out of
    /// them.
    pub fn assess_ocean(&mut self, idx: NodeIdx) -> bool {
        if idx.is_none() || !self.tree.nodes[idx.get()].alive {
            return false;
        }
        if self.sea_of(idx).is_some() {
            return true;
        }
        // A sea is not a planet, and has none of its own.
        ...
        // has it, and left air over the real one in no air at all. And a
        // planet without its surface has no ground to stand a sea on yet.
        match self.tree.nodes[idx.get()].morphology.as_ref() {
            None => return false,
            Some(m) if matches!(m.recipe.as_ref(), Some(crate::recipe::Recipe::Tiled(t)) if !t.is_ball()) => return false,
            Some(_) => {}
        }
        let mut up = self.tree.nodes[idx.get()].parent;
        while !up.is_none() {
            if self.sea_of(up).is_some() {
                return false;
            }
        ...
        // What the sea is made of: the planet's liquid, and nothing else.
        let mut water = crate::chem::Mixture::new();
        for p in self.tree.nodes[idx.get()].matter.mixture.entries() {
            if p.phase == crate::chem::Phase::Liquid {
                water.add(p.substance, p.phase, p.fraction / liquid);
            }
        }
        let composition = water.composition(&self.substances).0;
        // A shell's rms radius is its own, and `matter.radius` is the uniform
        // sphere with that rms (`sampler::radius_scale`).
        let shell = radius / (3.0f64 / 5.0).sqrt();
        let spec = self.tree.nodes[idx.get()].spec;
        let sea = self.tree.hold_out(idx, liquid_mass, composition, shell, spec);
        if sea.is_none() {
            return false;
        }
        let n = &mut self.tree.nodes[sea.get()];
    ...
    /// The node that is a planet's sea, if it has one: its child that carries
    /// an ocean.
    pub fn sea_of(&self, idx: NodeIdx) -> Option<NodeIdx> {
        self.tree.nodes[idx.get()]
            .children
            .iter()
            .copied()
            .find(|c| !c.is_none() && self.tree.nodes[c.get()].alive && self.tree.nodes[c.get()].ocean.is_some())
    }

    /// A planet's ocean: the description its sea's node carries.
    pub fn ocean_of(&self, idx: NodeIdx) -> Option<&crate::ocean::Ocean> {
        self.sea_of(idx).and_then(|s| self.tree.nodes[s.get()].ocean.as_deref())
    }

    /// [`World::ocean_of`], to change.
    pub fn ocean_of_mut(&mut self, idx: NodeIdx) -> Option<&mut crate::ocean::Ocean> {
        let sea = self.sea_of(idx)?;
        self.tree.nodes[sea.get()].ocean.as_deref_mut()
    }
    ...
    fn tides(&mut self, span: f64) {
        ...
        for i in 0..self.tree.nodes.len() {
            ...
            let stable = self.ocean_of(idx).map(|o| o.stable_step()).unwrap_or(0.0);
            if !(stable > 0.0) {
                continue;
            }
            ...
            let Some(ocean) = self.ocean_of_mut(idx) else { continue };
    ...
    pub fn winds_over(&self, idx: NodeIdx) -> Vec<(crate::ocean::Wind, NodeIdx)> {
        let mut out = Vec::new();
        let n = &self.tree.nodes[idx.get()];
        let Some(ocean) = self.ocean_of(idx) else { return out };
        let into = self.tree.facing(idx).conjugate();
    ...
    /// loses the momentum its stress carries and the kinetic energy that
    /// costs it, and the sea takes both — the water is what the wind pushes.
    /// The sea is a node whose matter cannot hold a push this small, so what
    /// it takes is written in its own books (`ocean::Account`), with the
    /// angular momentum about the sea's centre. The ocean's currents and sea
    /// are a description of part of that energy, not a second account of it.
    fn blow(&mut self, idx: NodeIdx, span: f64) {
        if !(span > 0.0) {
            return;
        }
        let Some(sea) = self.sea_of(idx) else { return };
        let winds = self.winds_over(idx);
        if !winds.is_empty() {
            let list: Vec<crate::ocean::Wind> = winds.iter().map(|w| w.0).collect();
            let blown = match self.ocean_of_mut(idx) {
            ...
            for ((w, air), b) in winds.iter().zip(blown.iter()) {
                ...
                o.account.momentum += dp;
                o.account.angular_momentum += (where_ - centre).cross(dp);
                o.account.energy += lost;
            }
        }
        // The sea travels, in steps it is stable at.
        let Some(ocean) = self.ocean_of_mut(idx) else { return };
        let mut left = span;
        while left > 0.0 {
    ...
    pub fn ambient_density(&self, idx: NodeIdx, r: f64) -> f64 {
        if let Some(o) = self.ocean_of(idx) {
            if r < o.radius {
    ...
    fn ground_of(&mut self, idx: NodeIdx, dt: f64) -> Option<crate::solvers::ground::Ground> {
        ...
        let floor = match self.tree.nodes[idx.get()].morphology.as_ref().and_then(|m| m.recipe.as_ref()) {
            Some(crate::recipe::Recipe::Tiled(t)) => t.floor(),
            _ => 0,
        };
        // The way the node's layout faces at its contents' instant — the turn
        // its drawing is placed at (`Tree::drawn_turn_at`).
        let facing = self.tree.drawn_turn_at(idx, self.tree.nodes[idx.get()].time);
        ...
        // patch of one pulls its pieces smoothly**, as the loose things among
        // them always were. A ball of seven pieces is where exact pairwise
        // pull is what keeps its momentum and angular momentum to 1e-15; a
        // patch is held by what is outside it, which takes up any net force a
        // smooth field leaves (`ground::Ground::held`), and the pieces of a
        // patch are continents stacked on each other: a cell and the column
        // 200 km under it, pulling as points, attract with the whole of the
        // cell's weight, and the stress holding them apart came to fifteen
        // times what the rock can carry.
        let pairwise = self.tree.nodes[idx.get()].parent.is_none();
        let gravity = {
            let n = &self.tree.nodes[idx.get()];
        ...
        if self.tree.nodes[idx.get()].ground.is_none() {
            ...
            for &(a, b, touch) in &joints {
                ...
                // shear the lattice needs, `G A / L` over the face the shear
                // acts across (`recipe::Touch`) — a side within a layer, the
                // whole footprint between one layer and the next — with `G` at
                // the Poisson's ratio of 0.3 `Material::yield_stress` takes.
                // Sized as a side's shear alone, the diagonals between a face's
                // cells and its columns let its corners slide at 6.9e-6 s^-2.
                let beside = 4.0 * h.x * h.z;
                let footprint = 4.0 * h.x * h.y;
                let k = match touch {
                    crate::recipe::Touch::Beside => stiffness * beside / length.max(1e-300),
                    crate::recipe::Touch::On => stiffness * footprint / length.max(1e-300),
                    crate::recipe::Touch::Across => {
                        let between = (a < floor) != (b < floor);
                        stiffness / 2.6 * if between { footprint } else { beside } / length.max(1e-300)
                    }
                };
                if length > 0.0 && k > 0.0 {
            ...
            let bare = Ground { springs: springs.clone(), field: field.clone(), preload: Vec::new(), gravity: if pairwise { Some(gravity) } else { None }, pieces, turn: Vec3::ZERO, lift: Vec::new(), swirl: None, held: false, floor: 0, foundation: Vec::new() };
            ...
            // Which floor piece bears each piece: itself, or what it rests on.
            let mut bears: Vec<usize> = (0..pieces).collect();
            for &(a, b, touch) in &joints {
                if touch == crate::recipe::Touch::On && a < floor && b >= floor && a < pieces {
                    bears[a] = b;
                }
            }
            let (tensions, outside, unresolved) = crate::solvers::ground::stress(&n.bodies, &springs, pieces, floor, &bears, &need);
            ...
            // What holds the whole — the pressure under a patch's floor, or
            // what the springs could not carry on a body resting on nothing —
            // as a support fixed in the ground. For ground something holds,
            // whatever net force and torque the pieces are left with beyond it
            // is taken off at the floor each step (`ground::Ground::held`).
            let support: Vec<Vec3> = outside
                .iter()
                .zip(&n.bodies)
                .map(|(f, b)| if b.mass > 0.0 { facing.conjugate().rotate(f.scale(1.0 / b.mass)) } else { Vec3::ZERO })
                .collect();
            // What the floor rests on, for ground something holds: the rock
            // beneath each floor piece, over as deep again as the piece is.
            let foundation: Vec<(usize, Vec3, f64)> = if n.parent.is_none() {
                Vec::new()
            } else {
                (floor.min(pieces)..pieces)
                    .filter_map(|i| {
                        let b = &n.bodies[i];
                        let h = b.half;
                        let (area, depth) = (4.0 * h.x * h.y, 2.0 * h.z);
                        (area > 0.0 && depth > 0.0 && stiffness > 0.0)
                            .then(|| (i, facing.conjugate().rotate(b.pos), stiffness * area / depth))
                    })
                    .collect()
            };
    ...
    fn ambient_in(&self, parent: NodeIdx, anc: NodeIdx, r: f64) -> f64 {
        if anc != parent {
            ...
            let below = self.ocean_of(anc).map(|o| r < o.radius).unwrap_or(false);
            if resolves && below {
                return 0.0;
            }
        }
    ...
    fn take_wrench(&mut self, idx: NodeIdx, dp: Vec3, dl: Vec3) -> f64 {
        ...
        // and the node is carried between solves in its holder's turning, so a
        // push kept on the node would be carried round with it rather than
        // staying where it was given. Put into the pieces, the weight of air
        // over a face of an Earth drove the face's contents off the face at
        // 8.7e-5 m/s^2; kept on the face, 1.1% of the momentum passing between
        ...
        if mass > 0.0 {
            ...
            let centre = takers.iter().fold(Vec3::ZERO, |c, t| c + t.2.scale(t.1)).scale(1.0 / mass);
            let mut inertia = crate::math::Mat3::zero();
            for t in &takers {
                let (m, d) = (t.1, t.2 - centre);
                let o = d.outer(d);
                for r in 0..3 {
                    for c in 0..3 {
